...
GlobalAmphib<-species_df

```

```{r}
#### Function to scrape data for each country###
scrape_country_data <- function(country_value) {
  # Construct the URL for the country
  url <- paste0(base_url, country_value)
  
  # Read the HTML content of the country-specific page
  web_page <- read_html(url)
  
  # Example: Extract species count or any other data you want
  species_count <- web_page %>%
    html_nodes(".species-count") %>%  # Adjust with the correct selector for species count
    html_text() %>%
    trimws()  # Clean up the text
  
  # Return the species count or any other extracted data
  return(species_count)
}

# Loop through the country values and scrape the data
country_data <- lapply(country_values, scrape_country_data)
