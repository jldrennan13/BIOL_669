...
caudata_sf<-st_as_sf(caudata)

#Check spatial specifics
class(caudata)
names(caudata)
caudata$sci_name<-as.factor(caudata$sci_name)
...
species_ranges_genus <- caudata_sf %>% 
  filter(genus %in% c("Ambystoma"))

#Map relevant ranges 
ggplot() +
  geom_map(data = world_map, map = world_map, aes(x = long, y = lat, map_id = region), 
           color = "black", fill = "lightgrey", size = 0.2) +
  geom_sf(data = species_ranges_genus, aes(fill = sci_name), color = "transparent",alpha=0.5) +
  theme_minimal() +
...
# Calculate the area for each species' range
species_ranges_genus$area <- st_area(species_ranges_genus)

# Summarize total area for each species
species_range_sizes <- species_ranges_genus %>%
  group_by(sci_name) %>%
  summarize(range_size = sum(area, na.rm = TRUE))  
